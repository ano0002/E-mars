...
class Particle:
    def __init__(self, pos, vel, timer,color,radius,gravity,glow_color,glow_radius=0):
        ...
        self.surface = circle_surf(self.glow_radius, self.glow_color,self.alpha)
    ...
    def draw(self,screen):
        if self.glow_radius != self.surface.get_width():
            self.surface = circle_surf(self.glow_radius, self.glow_color,self.alpha)
        screen.blit(self.surface, (self.pos[0]-self.glow_radius, self.pos[1]-self.glow_radius), special_flags=pygame.BLEND_ADD)
...
class Torch(Particle_Emitter):
    def __init__(self, tile_pos,offset):
        ...
        self.offset = offset
        self.default_pos = tile_pos
    def update(self,offset):
        self.offset = offset
        self.pos = [
                self.default_pos[0] + self.offset[0],
                self.default_pos[1] + self.offset[1]
        ]
        super().update()
